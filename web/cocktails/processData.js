...
var writeValueWhereKey = function(tree, searchKey, searchValue, writeKey, writeValue) {
  // writeValueWhereKey()
...
function appendReductionToObjTreeLeaves(obj, depth) {
  ...
  for (var i = 0; i < leafPaths.length; i++) {
    ...
    if (match.length > 1) {
      ...
      _.set(obj, path + '.children', [{ name: match, children: [], class: 'match' }])
    } else {
...
appendReductionToObjTreeLeaves([A.treeData2], 5)
